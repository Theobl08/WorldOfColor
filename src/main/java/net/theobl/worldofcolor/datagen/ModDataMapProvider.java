...
public class ModDataMapProvider extends DataMapProvider {
    ...
    protected void gather(HolderLookup.Provider provider) {
        ...
        for(DeferredBlock<Block> block : ModBlocks.COLORED_COPPER_GOLEM_STATUES) {
            ...
            oxidizables.add(block, new Oxidizable(ModBlocks.COLORED_COPPER_GOLEM_STATUES.get(index + 1).get()), false);
        }
        ...
        for(DeferredBlock<Block> block : ModBlocks.COLORED_COPPER_GOLEM_STATUES) {
            int index = ModBlocks.COLORED_COPPER_GOLEM_STATUES.indexOf(block);
            waxables.add(block, new Waxable(ModBlocks.COLORED_WAXED_COPPER_GOLEM_STATUES.get(index).get()), false);
        }
        for(DeferredBlock<Block> block : ModBlocks.COLORED_LIGHTNING_RODS) {
            int index = ModBlocks.COLORED_LIGHTNING_RODS.indexOf(block);
